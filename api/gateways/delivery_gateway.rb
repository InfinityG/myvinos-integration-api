...
class DeliveryGateway
  ...
  def send_delivery_request(user, order)
      ...
      begin
        ...
        raise ApiError, "#{THIRD_PARTY_DELIVERY_REQUEST_ERROR} | Response code: #{response.response_code}" if response.response_code != 200
        return response
      rescue RestClient::Exception => e
        raise ApiError, "#{THIRD_PARTY_DELIVERY_REQUEST_ERROR}: #{e.http_code} | #{e.http_body}"
      end
  end
end