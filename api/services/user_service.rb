...
class UserService
  ...
  def create_or_update(validated_auth)
    ...
    if user != nil && user.third_party_id.to_s != '' # user already exists, so update fields if required
      update_user(email, first_name, last_name, mobile_number, user)
    else # create new user
      create_user(email, external_id, first_name, last_name, meta, mobile_number, username)
    end

    # TODO: create new vinos_bonus order type
  end

  def create_user(email, external_id, first_name, last_name, meta, mobile_number, username)
    ...
    @user_repository.create external_id, third_party_id, username, first_name, last_name, email,
                            mobile_number, meta, start_balance
  end

  def update_user(email, first_name, last_name, mobile_number, user)
    updated_email = user.email.to_s.downcase != email.to_s.downcase ? email : nil
    updated_first_name = user.first_name != first_name ? first_name : nil
    updated_last_name = user.last_name != last_name ? last_name : nil
    updated_mobile_number = user.mobile_number != mobile_number ? mobile_number : nil

    # only update if any of the fields have changed
    if updated_email != nil || updated_first_name != nil || updated_last_name != nil || updated_mobile_number != nil
      # update on the third party
      @product_gateway.update_user user.third_party_id, updated_email, updated_first_name,
                                   updated_last_name, updated_mobile_number

      # update on the local repo
      return @user_repository.update user, first_name, last_name, email, mobile_number
    end
